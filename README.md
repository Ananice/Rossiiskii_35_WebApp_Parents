...
## Демо (Render)
URL: https://muiv-college-parents-portal.onrender.com/login/

Демо-аккаунты: admin1, staff1, parent1  
Пароль: Admin12345!

Примечание: на free-плане Render недоступны Shell/SSH, поэтому демо-аккаунты создаются автоматически при деплое (см. AUTO_CREATE_DEMO_USERS).
...
- Указать строку подключения к БД PostgreSQL в формате `DATABASE_URL`
  (пример: postgresql://user:password@host:5432/dbname)
- Указать `SECRET_KEY` (допускается `DJANGO_SECRET_KEY`)
...
На free-плане Render недоступны Shell/SSH, поэтому демо-аккаунты нужно создавать автоматически во время деплоя
(через build.sh) или заранее локально до деплоя.
